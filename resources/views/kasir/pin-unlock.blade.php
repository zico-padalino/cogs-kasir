...
<body class="login-page kasir-pin-page" data-kasir-notifications data-kasir-poll-url="{{ route('kasir.pending.poll') }}" data-kasir-poll-interval="{{ config('pos.notifications.poll_interval_seconds', 20) }}" data-kasir-auto-load="0" data-kasir-pin-poll-only="1">
    ...
    <div class="login-card kasir-pin-card">
        ...
        @endif
        @if (session('error'))
            <div class="mb-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">{{ session('error') }}</div>
        @endif
        @if ($errors->any())
        ...
        <form action="{{ route('kasir.pin.unlock.submit') }}" method="POST" class="space-y-3" autocomplete="off" id="kasir-pin-form">
            ...
            <div>
                ...
                <input
                    ...
                    placeholder="••••"
                >
                <p class="mt-1.5 text-center text-[11px] text-slate-500">Isi PIN — otomatis masuk tanpa klik tombol</p>
            </div>
        ...
        <div class="mt-3 space-y-2 border-t border-slate-200 pt-3">
            ...
            <form action="{{ route('logout') }}" method="POST">
                ...
                <button
                    type="submit"
                    class="btn-outline w-full min-h-10 border-slate-300 text-slate-700 hover:border-red-300 hover:bg-red-50 hover:text-red-700"
                >
                    Keluar / Logout
                </button>
            </form>
