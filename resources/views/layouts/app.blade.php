...
<body class="app-body is-cogs min-h-screen bg-[#f6f1ea] font-sans text-slate-900 antialiased">
    ...
    <div class="app-shell">
        ...
        <div class="app-content flex min-h-0 min-w-0 flex-1 flex-col md:pl-64">
            @include('layouts.partials.mobile-topbar', [
                'moduleLabel' => 'COGS',
                'fallbackHeading' => 'Hitung Modal',
                'defaultSubtitle' => auth()->check() ? auth()->user()->name : null,
            ])

            <header class="page-header sticky top-0 z-20 hidden shrink-0 border-b border-slate-200 bg-white/90 backdrop-blur md:block">
                <div class="page-header-inner page-header-inner--wide">
                    <div class="flex min-w-0 items-center gap-3">
                        ...
                        <div class="min-w-0">
                            <h1 class="page-header-title">@yield('heading')</h1>
                            @hasSection('subheading')
                                <p class="page-header-sub">@yield('subheading')</p>
                            @endif
                        </div>
                    </div>
                    @if (! ($setupFullyComplete ?? false))
                        <a href="{{ route('dashboard') }}" class="page-header-link">← Panduan</a>
                    @else
                        <a href="{{ route('dashboard') }}" class="page-header-link">← Beranda</a>
                    @endif
                </div>
            </header>
