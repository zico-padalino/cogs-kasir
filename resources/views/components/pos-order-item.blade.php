...
@php
    ...
    $customerNote = $noteParts['customer'] ?? '';
    $hasCustomerNote = filled($customerNote);
@endphp
...
>
    ...
    @if ($editable && $updateUrl)
        <details class="pos-order-item-note-details">
            <summary class="pos-order-item-note-summary {{ $hasCustomerNote ? 'has-note' : '' }}">
                @if ($hasCustomerNote)
                    <span class="pos-order-item-note-summary-label">Catatan</span>
                    <span class="pos-order-item-note-summary-text">{{ $customerNote }}</span>
                    <span class="pos-order-item-note-summary-action">Ubah</span>
                @else
                    <span class="pos-order-item-note-summary-action">+ Catatan</span>
                @endif
            </summary>
            <form action="{{ $updateUrl }}" method="POST" class="pos-item-note-form pos-order-item-note-form">
                @csrf
                @method('PATCH')
                <label class="pos-item-note-label" for="item-note-{{ $item->id }}">Catatan item</label>
                <textarea
                    ...
                    placeholder="Contoh: less sugar, hot, tanpa es"
                >{{ old('notes', $customerNote) }}</textarea>
                <div class="pos-order-item-note-actions">
                    <button type="submit" class="pos-item-note-save">Simpan catatan</button>
                </div>
            </form>
        </details>
    @elseif ($hasCustomerNote)
        <p class="order-item-note-display pos-order-item-note-readonly">{{ $customerNote }}</p>
    @endif
</div>
